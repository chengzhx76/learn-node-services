...
function expressionToHtml(elem: SlateElement, childrenHtml: string): string {
  ...
  const expressionStr = encodeURIComponent(JSON.stringify(list))
  return `<a data-w-e-type="expression" expression="${expressionStr}" data-w-e-is-void data-w-e-is-inline">${role}</a>`
}
